...
const Button = React.forwardRef<HTMLButtonElement, ButtonProps>(
  ({ variant = 'primary', size = 'md', isLoading = false, fullWidth = false, children, className, disabled, ...props }, ref) => {
    ...
    const variantClasses: Record<ButtonVariant, string> = {
      ...
      primary:
        'bg-gradient-to-r from-purple-500 to-indigo-600 text-white shadow-md hover:shadow-lg hover:from-purple-600 hover:to-indigo-700 active:from-purple-700 active:to-indigo-800 disabled:bg-slate-300 disabled:text-slate-500',
      // Secondary variant - white surface with border
      secondary:
        'bg-white border border-purple-200 text-purple-700 hover:bg-purple-50 hover:border-purple-300 dark:bg-purple-900/30 dark:border-purple-700 dark:text-purple-100 dark:hover:bg-purple-800/50',
      // Ghost variant - transparent with hover background
      ghost:
        'bg-transparent text-purple-600 hover:bg-purple-100 dark:text-purple-300 dark:hover:bg-purple-900/50',
      // Danger variant - rose for destructive actions
      danger:
        'bg-gradient-to-r from-rose-500 to-pink-600 text-white shadow-md hover:shadow-lg hover:from-rose-600 hover:to-pink-700 active:from-rose-700 active:to-pink-800',
    };
